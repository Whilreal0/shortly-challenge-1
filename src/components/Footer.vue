<template>
  ...
  >
   <div class="lg:pt-[80px]
      ...
      lg:pb-[20px]
      lg:grid
      lg:grid-cols-[1fr_repeat(4,auto)]
      lg:items-start
      lg:gap-[70px]
      lg:text-left 
      xl:px-[125px]
     "
       id="lg-grid ">
     <div class="flex justify-center items-center py-[15px] lg:p-0 xl:flex-col xl:items-start">
    ...
    <div
      class="
        ...
        lg:text-left
      "
      v-for="(items, i) in FooterItems"
      :key="i"
    >
      <h3 class="text-[18.72px] mb-[10px] lg:text-[16px] lg:w-full">
        {{ items.Title }}
      </h3>
      <ul class="flex flex-col gap-[10px]">
        <li class="text-[14.4px] text-[#9e9aa7] hover:text-[#2ACFCF] cursor-pointer">
          {{ items.Item[0] }}
        </li>
        <li class="text-[14.4px] text-[#9e9aa7] hover:text-[#2ACFCF] cursor-pointer">
          {{ items.Item[1] }}
    ...
    <div class="flex items-center justify-center gap-4 mt-10">
      <box-icon class="cursor-pointer hover:fill-[#2ACFCF] fill-white"  type="logo" name="facebook-square"></box-icon>
      <box-icon class="cursor-pointer hover:fill-[#2ACFCF] fill-white"  name="twitter" type="logo"></box-icon>
      <box-icon class="cursor-pointer hover:fill-[#2ACFCF] fill-white"  name="pinterest" type="logo"></box-icon>
      <box-icon class="cursor-pointer hover:fill-[#2ACFCF] fill-white"  name="instagram" type="logo"></box-icon>
    </div>
   </div>
    <div class="text-center mt-[2.5rem]" id="attribution ">
      <span class="text-[11px] text-[#9e9aa7]"
        >Challenge by
        <a class="font-medium text-white" href="https://www.frontendmentor.io/">
          Frontend Mentor</a
        >
        Coded by Whilmer.</span
      >
    </div>
  </footer>
</template>
...
export default {
  data: () => ({
    FooterItems: [
      {
        Title: "Features",
        Item: ["Link Shortening", "Branded Links", "Analytics"],
      },
      {
        Title: "Resources",
        Item: ["Blog", "Developers", " Support"],
      },
      {
        Title: "Company",
        Item: ["About", "Our Team", "Careers", "Contact"],
      },
    ],
    Features: [
      {
